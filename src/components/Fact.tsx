                            ...
                            {fact.title ? fact.title :
                                <>L'évènement {sortedDates.length > 1 ? 'des ' : 'du '}
                                    {sortedDates.map((date, index) => {
                                        ...
                                        return (
                                            <>
                                                <span key={date.getTime()} className='date'>
                                                    {index > 0 ? (index === sortedDates.length - 1 ? ' et ' : ', ') : ''}
                                                    {formattedDate}
                                                </span>
                                                {index === sortedDates.length - 1 ? (<> dans <span key={fact.location.id} className='location'>{getLocationText(fact.location.type, fact.location.name)}</span></>) : ''}
                                            </>
                                        );
                                    })}</>}
                        </h1>
